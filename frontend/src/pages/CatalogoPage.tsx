    ...
    return (
        <div className="max-w-7xl mx-auto px-3 py-4 md:p-4 space-y-4 pb-20 md:pb-4">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                ...
                {isEditor && (
                    <div className="flex gap-2">
                        ...
                        <button onClick={() => setIsUpdatePricesModalOpen(true)} className="bg-white hover:bg-gray-50 text-gray-700 border border-gray-200 px-4 py-2.5 rounded-xl font-medium text-sm flex items-center justify-center gap-2 shadow-sm transition-colors">
                            <FileSpreadsheet size={18} className="text-blue-600" />
                            <span className="hidden lg:inline">Act. Precios por Sucursal</span>
                        </button>
                        <button onClick={() => setIsImportModalOpen(true)} className="bg-white hover:bg-gray-50 text-gray-700 border border-gray-200 px-4 py-2.5 rounded-xl font-medium text-sm flex items-center justify-center gap-2 shadow-sm transition-colors">
                            ...
                            <span className="hidden md:inline">Importar Básica</span>
                        </button>
            ...
            {isUpdatePricesModalOpen && (
                <UpdatePricesModal onClose={() => setIsUpdatePricesModalOpen(false)} sucursales={sucursales} />
            )}
        </div>
    );
}
...
function UpdatePricesModal({ onClose, sucursales }: { onClose: () => void, sucursales: Sucursal[] }) {
    const queryClient = useQueryClient();
    const [isUploading, setIsUploading] = useState(false);
    const [result, setResult] = useState<any>(null);
    const [error, setError] = useState<string | null>(null);
    ...
    const onDrop = async (acceptedFiles: File[]) => {
        ...
        setError(null);
        try {
            const data = await importProductPrices(selectedSucursalId, file);
            setResult(data);
            queryClient.invalidateQueries({ queryKey: ['products'] });
        } catch (err: any) {
            setError(err.message || 'Error al procesar el archivo');
    ...
    const { getRootProps, getInputProps, isDragActive } = useDropzone({
        ...
        maxFiles: 1,
        disabled: isUploading || !!result || !selectedSucursalId
    });

    const handleDownloadTemplate = async () => {
        if (!selectedSucursalId) {
            setError('Primero selecciona una sucursal para descargar su plantilla');
            return;
        }
        try {
            setError(null);
            await exportProductPriceTemplate(selectedSucursalId);
        } catch (err) {
            setError("Error descargando la plantilla de precios");
        }
    }
                    ...
                    {!result ? (
                        <div className="space-y-6">
                            <div className="bg-blue-50/50 p-5 rounded-xl border border-blue-100">
                                <h4 className="font-semibold text-blue-900 mb-2">Instrucciones</h4>
                                <ol className="list-decimal list-inside text-sm text-blue-800 space-y-1.5 ml-1">
                                    <li>Selecciona la sucursal que deseas actualizar.</li>
                                    <li>Descarga la plantilla, la cual te mostrará el precio actual de la sucursal elegida.</li>
                                    <li>Modifica la columna <b>NUEVO PRECIO</b>. No modifiques el <b>CODIGO CORTO</b>.</li>
                                    <li>Sube el archivo final.</li>
                                </ol>
                            </div>

                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-2">1. Seleccionar Sucursal Empresa</label>
                                <div className="flex gap-4">
                                    <select
                                        className="flex-1 bg-gray-50 border border-gray-200 focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 rounded-xl px-4 py-2.5 outline-none transition-all text-sm text-gray-900"
                                    ...
                                    <button 
                                        ...
                                        disabled={!selectedSucursalId || isUploading}
                                        className="flex items-center gap-2 px-5 py-2 min-w-max bg-blue-600 hover:bg-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white rounded-xl text-sm font-medium transition-colors shadow-sm"
                                    >
                                        <Download size={18} />
                                        Descargar Plantilla
                                    </button>
                            ...
                            <div 
                                {...getRootProps()} 
                                className={`border-2 border-dashed rounded-2xl p-8 text-center transition-all cursor-pointer flex flex-col items-center justify-center min-h-[160px] mt-4
                                    ${isDragActive ? 'border-blue-500 bg-blue-50' : 'border-gray-300'}
                                    ${(!selectedSucursalId || isUploading) ? 'opacity-50 pointer-events-none bg-gray-50' : 'hover:border-blue-400 hover:bg-gray-50'}`}
                            >
                                <input {...getInputProps()} />
                                {isUploading ? (
                                    <div className="flex flex-col items-center text-blue-600">
                                        <Loader2 size={32} className="animate-spin mb-3" />
                                        <p className="font-medium">Actualizando precios de forma segura...</p>
                                    </div>
                                ) : (
                                    <>
                                        <div className="w-12 h-12 bg-blue-100 text-blue-600 rounded-xl flex items-center justify-center mb-3 text-blue-600">
                                            <Upload size={24} />
                                        </div>
                                        <p className="font-semibold text-gray-700 mb-1">2. Sube la plantilla con los nuevos precios</p>
                                        <p className="text-sm text-gray-500">Mismo formato Excel</p>
                                    </>
                                )}
                            </div>

                            {error && (
                                <div className="p-4 bg-red-50 text-red-700 text-sm rounded-xl border border-red-200 font-medium flex items-start gap-2 mt-4">
                                    <X size={18} className="shrink-0 mt-0.5" />
                                    <span>{error}</span>
                                </div>
                            )}
                        ...
                        <div className="space-y-6">
                            <div className="bg-emerald-50 border border-emerald-200 p-5 rounded-xl">
                                ...
                                <div className="grid grid-cols-4 gap-4 text-center">
                                    <div className="bg-white p-3 rounded-lg border border-emerald-100 shadow-sm">
                                        <div className="text-2xl font-black text-gray-900">{result.resumen.filas_leidas}</div>
                                        <div className="text-[10px] text-gray-500 font-medium uppercase mt-1">Leídas</div>
                                    </div>
                                    <div className="bg-white p-3 rounded-lg border border-emerald-100 shadow-sm">
                                        <div className="text-2xl font-black text-emerald-600">{result.resumen.precios_actualizados}</div>
                                        <div className="text-[10px] text-emerald-600/70 font-medium uppercase mt-1">Actualizados</div>
                                    </div>
                                    <div className="bg-white p-3 rounded-lg border border-emerald-100 shadow-sm">
                                        <div className="text-2xl font-black text-gray-400">{result.resumen.filas_ignoradas}</div>
                                        <div className="text-[10px] text-gray-400 font-medium uppercase mt-1">Ignoradas</div>
                                    </div>
                                    <div className="bg-white p-3 rounded-lg border border-emerald-100 shadow-sm">
                                        <div className="text-2xl font-black text-red-600">{result.resumen.errores}</div>
                                        <div className="text-[10px] text-red-600/70 font-medium uppercase mt-1">Errores</div>
                                    </div>
                                </div>
                            </div>
