...
async def get_general_reports(
    ...
    kpis_cursor = await SaleItem.aggregate(kpis_pipeline).to_list(length=1)
    ...
    ventas_por_sucursal = await SaleItem.aggregate(sucursal_pipeline).to_list(length=100)
    ...
    top_productos = await SaleItem.aggregate(top_products_pipeline).to_list(length=10)
    ...
    evolucion_diaria = await SaleItem.aggregate(diaria_pipeline).to_list(length=100)
