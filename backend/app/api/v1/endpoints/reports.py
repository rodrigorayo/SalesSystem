...
async def get_general_reports(
    ...
    top_productos = await SaleItem.get_pymongo_collection().aggregate(top_products_pipeline).to_list(length=10)
    ...
    evolucion_diaria = await SaleItem.get_pymongo_collection().aggregate(diaria_pipeline).to_list(length=100)
